...
name = ''
spisok_films_500 = []
spisok_films_random = [set()]
def spisok_random():
    global spisok_films_random
    if len(spisok_films_random) == 0:
        for i in range(1, 20):
            response_get = requests.get(f'https://www.kinopoisk.ru/lists/movies/popular-films/?page={i}')
            soup = bs(response_get.text, features='html.parser')
            quotes_films = soup.find_all('span', class_='styles_mainTitle__IFQyZ styles_activeMovieTittle__kJdJj')
            for film in quotes_films:
                spisok_films_random.append(film.text)
        return random.choice(spisok_films_random)
    else:
        return random.choice(spisok_films_random)


def spisok_500():
    global spisok_films_500
    if len(spisok_films_500) == 0:
        for i in range(1, 10):
            response_get = requests.get(f'https://www.kinopoisk.ru/lists/movies/top500/?page={i}')
            soup = bs(response_get.text, features='html.parser')
            quotes_films = soup.find_all('span', class_='styles_mainTitle__IFQyZ styles_activeMovieTittle__kJdJj')
            for film in quotes_films:
                spisok_films_500.append(film.text)
        return random.choice(spisok_films_500)
    else:
        return random.choice(spisok_films_500)
...
def keyboard_pusk():
    ...
    button_genge = telebot.types.KeyboardButton('Жанры фильмов')
...
def keyboard_genre():
    ...
    key_mult= telebot.types.InlineKeyboardButton(text='Мультфильмы', callback_data='comedy')
    key_comedy = telebot.types.InlineKeyboardButton(text='Комедии', callback_data='mult')
    key_fantasy = telebot.types.InlineKeyboardButton(text='Фэнтази', callback_data='fantasy')
    key_horror = telebot.types.InlineKeyboardButton(text='Ужас', callback_data='horror')
...
def genre_reply(message):
    ...
    if message.text == 'Случайный фильм из популярного':
        bot.send_message(message.chat.id, spisok_random())
...
def genre_reply_but(call):
    ...
    if call.data == 'fantasy':
        bot.send_message(call.message.chat.id, 'Вы выбрали фэнтази')
    if call.data == 'horror':
